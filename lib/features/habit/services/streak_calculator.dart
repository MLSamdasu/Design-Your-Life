...
// 하루라도 체크하지 않으면 currentStreak가 0으로 리셋된다.
...
abstract class StreakCalculator {
  ...
  static StreakResult calculate(
    ...
    // 현재 스트릭 계산 (오늘 또는 어제부터 역방향 연속 체크)
    ...
    if (completedDates.contains(todayStart)) {
      ...
      return StreakResult(
        currentStreak: 0,
        longestStreak: _calculateLongest(completedDates),
      );
    }
  ...
  /// 최장 스트릭을 계산한다
  static int _calculateLongest(Set<DateTime> dates) {
